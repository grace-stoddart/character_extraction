...
def QU(chainHeads, ann):

    QUFeature = np.zeros(len(chainHeads))

    for j, head in enumerate(chainHeads):

        for quote in ann['quotes']:

            if head.lower().strip() in quote["speaker"].lower().strip():
                QUFeature[j] += 1

    QUFeatureNormed = (QUFeature - np.mean(QUFeature)) / np.std(QUFeature)
...
def SS(chainHeadOfHeads, semanticSubjects):
    ...
    SSFeatureNormed = (SSFeature - np.mean(SSFeature))/np.std(SSFeature)              
...
def DP(chainHeadOfHeads, dependencies):
    ...
    DPFeatureNormed = (DPFeature - np.mean(DPFeature))/np.std(DPFeature)              
...
def TP(chainHeadOfHeads, tripleSubjects):
    ...
    for i in range(n):
        ...
        if v>0 and isPronoun==False:
            TPFeature[i] = v

    TPFeatureNormed = (TPFeature - np.mean(TPFeature))/np.std(TPFeature)              
